...
primitive_h :: LogicFunction -> Delay -> GateState -> [EventSequence] -> EventSequence
primitive_h logic_fn delay gate_init isq =
  ...
  in if minind < 0 
       then [] -- If no events
                ...
                next_seq = take minind isq ++ [(tail (isq !! minind))] ++ drop (minind + 1) isq
                oop = logic_fn gate_init
                next_gate_init = take minind gate_init ++ [(snd ev)] ++ drop (minind + 1) gate_init
                nop = logic_fn next_gate_init 
            in (delay + (fst ev), nop):(primitive_h logic_fn delay next_gate_init next_seq)
...
pick_min isq = pick_min_h isq inf (-1) 0
